<?php
add_action('customize_register', 'lavan_customize_register');
function lavan_customize_register($wp_customize) {
	...
	$wp_customize->remove_section( 'sidebar-widgets-left-sidebar' );
	$wp_customize->remove_section( 'themes' );
	}
// Sanitize Functions number validation	
function check_number( $input ) {
    $input = (int) $input; // Force the value into integer type.
		...
		public function render_content(){
	        ...
	        <label>
	        	...
	        	<div class="customize_img_upload">
					...
					<input type="hidden">
					<div class="buttons">
						<a href="#" class='upload_media_image button'><?php _e('Upload Image','spa') ?></a>
						<a href="#" class='remove_media_image button'><?php _e('Remove','spa') ?></a>
					</div> 
				</div>
	        </label>	        
	        <?php
	...
	public function render_content()
    {
        if(!empty($this->pages))
        {
            ?>
                <label class="customize-control-title"><?php echo esc_html( $this->label ); ?></label>
                    <select <?php $this->link(); ?> name="<?php echo $this->id; ?>" id="<?php echo $this->id; ?>">
                    <option value="select-page"><?php _e('Select Page Footer','spa') ?></option>
                    <?php
...
class Kaya_Customize_google_fonts_Control extends WP_Customize_Control
{
	...
	public function render_content(){ ?>
		...
		<select <?php $this->link(); ?> style="">
			<?php
			$defaylt_fonts = array ('0' => __('Select Font','spa'),
...
function globel_font_family(){
	...
	$customizer_font_names = array($header_text_logo_font_family, $google_bodyfont, $google_menufont, $google_generaltitlefont, $text_logo_tagline_font_family, $page_titlebar_dec_font_family, $page_titlebar_font_family );
